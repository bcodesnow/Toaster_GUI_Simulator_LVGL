...
static void lv_spinbox_event_cb(lv_obj_t * obj, lv_event_t event);
...
lv_obj_t * lv_spinbox_create(lv_obj_t * par, const lv_obj_t * copy)
{
    ...
    lv_ta_set_cursor_type(new_spinbox, LV_CURSOR_NONE);
    ...
    /*Copy an existing spinbox*/
        ...
        lv_spinbox_set_digit_format(new_spinbox, (uint8_t)copy_ext->digit_count, (uint8_t)copy_ext->dec_point_pos, (uint8_t)copy_ext->flags);
 ...
 * shown
 */
void lv_spinbox_set_digit_format(lv_obj_t * spinbox, uint8_t digit_count, uint8_t separator_position, uint8_t minutes_n_seconds)
{
    LV_ASSERT_OBJ(spinbox, LV_OBJX_NAME);
...
//    if ( minutes_n_seconds )
//        ext->flags |= LV_SPINBOX_MIN_N_SECS_POS;
//    else
//        ext->flags &= ~LV_SPINBOX_MIN_N_SECS_POS;
    ext->flags |= minutes_n_seconds;
...
void lv_spinbox_increment(lv_obj_t * spinbox)
{
    ...
    lv_spinbox_ext_t * ext = lv_obj_get_ext_attr(spinbox);
    if (ext->flags & LV_SPINBOX_EN_POS )
    {
...
static void lv_spinbox_updatevalue(lv_obj_t * spinbox)
{
    ...
    //printf("lv_spinbox_updatevalue - ext->val = %d \n", ext->value );
    //printf("FLAGS: %d", ext->flags);
    char buf[LV_SPINBOX_MAX_DIGIT_COUNT + 8];
    memset(buf, 0, sizeof(buf));
    char * buf_p = buf;
    uint8_t cur_shift_left = 0;

    if ( ext->flags & LV_SPINBOX_CURSOR_EN_POS )
    {
        //printf("\nupd curs en \n");
        lv_ta_set_cursor_type(spinbox, LV_CURSOR_BLOCK);
    }
    else
    {
        //printf("\nupd curs dis \n");

        lv_ta_set_cursor_type(spinbox, LV_CURSOR_NONE);
    }

    if ( ext->flags & LV_SPINBOX_EN_POS )
    {
        lv_obj_set_click(spinbox, 1);
        lv_obj_set_click(lv_page_get_scrl(spinbox), true);
    }
    else
    {
        lv_obj_set_click(spinbox, 0);
    ...
    int32_t intDigits;
    if ( !(ext->flags & LV_SPINBOX_MIN_N_SECS_POS) )
    {
        char digits[64];
    ...
    {
        // this is a hack in place solution to have a timer..
        uint32_t val = ext->value;
        uint8_t minutes = val / 60;
        uint8_t seconds = val % 60; // ext->value - (minutes * 60);
        uint8_t digits[4];
        digits[0] = minutes / 10;
        digits[1] = minutes % 10;
        digits[2] = seconds / 10;
        digits[3] = seconds % 10;
        lv_utils_num_to_str( digits[0], buf_p++);
        lv_utils_num_to_str( digits[1], buf_p++);
        *(buf_p++) = ':';
        lv_utils_num_to_str( digits[2], buf_p++);
        lv_utils_num_to_str( digits[3], buf_p++);
        intDigits = 2;
    }
...
#define SPINBOX_CURSOR_FIX_POS 1
#if !SPINBOX_CURSOR_FIX_POS
...
#else
    lv_ta_set_cursor_pos(spinbox, 6);
#endif
...
void lv_spinbox_set_enabled(lv_obj_t *spinbox, uint8_t enabled)
{
    lv_spinbox_ext_t * ext = lv_obj_get_ext_attr(spinbox);

    if ( enabled )
        ext->flags |= LV_SPINBOX_EN_POS;
    else
        ext->flags &= ~LV_SPINBOX_EN_POS;
...
uint8_t lv_spinbox_get_enabled(lv_obj_t *spinbox)
{
    lv_spinbox_ext_t * ext = lv_obj_get_ext_attr(spinbox);
...
void lv_spinbox_set_cursor_enabled(lv_obj_t *spinbox, uint8_t enabled)
{
    lv_spinbox_ext_t * ext = lv_obj_get_ext_attr(spinbox);

    if ( enabled )
        ext->flags |= LV_SPINBOX_CURSOR_EN_POS;
    else
        ext->flags &= ~LV_SPINBOX_CURSOR_EN_POS;
...
uint8_t lv_spinbox_get_cursor_enabeld(lv_obj_t *spinbox)
{
    lv_spinbox_ext_t * ext = lv_obj_get_ext_attr(spinbox);
